...
def create_clients(config):
    print('1.0 Remove a Client Redshift and IAM')
...
def empty_bucket(config):
    print('1.3 Clean objects bucket {}'.format(config["S3"]["bucket_name"]))
...
def delete_bucket(config):
    print('1.4 Remove bucket {}'.format(config["S3"]["bucket_name"]))
    client = boto3.client('s3',
                      region_name=config['AWS']['REGION'],
                      aws_access_key_id=config['AWS']['KEY'],
                      aws_secret_access_key=config['AWS']['SECRET'])
    try:
        ...
        print("1.4.1 Bucket {} delete !".format(config["S3"]["bucket_name"]))
    except e:
        print("Erro to delete bucket")

def update_cfg():
    print('1.5 Remove data config')
